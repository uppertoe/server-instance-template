...
check_remote "timesyncd config includes expected NTP servers" "grep -F 'NTP=time.cloudflare.com time.google.com' /etc/systemd/timesyncd.conf >/dev/null && grep -F 'FallbackNTP=0.pool.ntp.org 1.pool.ntp.org' /etc/systemd/timesyncd.conf >/dev/null"
