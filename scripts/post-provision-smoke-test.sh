...
check_remote "pam_faillock preauth line includes lockout parameters" "grep -F 'pam_faillock.so preauth silent audit deny=4 fail_interval=900 unlock_time=900' /etc/pam.d/common-auth >/dev/null"
check_remote "pam_faillock authfail line includes lockout parameters" "grep -F 'pam_faillock.so authfail audit deny=4 fail_interval=900 unlock_time=900' /etc/pam.d/common-auth >/dev/null"
