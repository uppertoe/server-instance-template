...
| auth service | **disabled on staging** — when enabled: operator email, session/TOTP secrets | yes (operator only) | 2 | 2 | 1 | OFFICIAL: Sensitive | `auth_data` volume (AES-GCM for reversible secrets) | env-files restic service | operational |
